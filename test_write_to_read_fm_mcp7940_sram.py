...
def main():
    ...
    print("RTC status: is started? ", "Yes" if mcp.is_started() else "No")
    print("Status backup battery: enabled? ", "Yes" if mcp.is_battery_backup_enabled() else "No")
    ...
    if not bbu_was_enabled:
        if mcp.is_battery_backup_enabled():
            ...
            print("Status backup battery 2nd check: enabled? ", "Yes" if mcp.is_battery_backup_enabled() else "No")
    else:
